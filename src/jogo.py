...
def criar_jogo(data, estadio, id_clube_casa, id_clube_fora, golos_casa=0, golos_fora=0):
    ...
    if id_clube_casa == id_clube_fora:
        logger.warning("Erro ao criar jogo: clube da casa e visitante são o mesmo (ID %s)", id_clube_casa)
...
def listar_jogos():
    carregar_jogos()
    if not jogos:
        logger.warning("Listagem de jogos: nenhum jogo registado")
...
def consultar_jogo(id_jogo):
    ...
    if id_jogo not in jogos:
        logger.warning("Jogo com ID %s não encontrado", id_jogo)
...
def atualizar_jogo(id_jogo, golos_casa=None, golos_fora=None, estadio=None):
    carregar_jogos()
    if id_jogo not in jogos:
        logger.warning("Atualização falhada: jogo com ID %s não encontrado", id_jogo)
        return 404, "Jogo não encontrado."
...
def remover_jogo(id_jogo):
    carregar_jogos()
    if id_jogo not in jogos:
        logger.warning("Remoção falhada: jogo com ID %s não encontrado", id_jogo)
        return 404, "Jogo não encontrado."
